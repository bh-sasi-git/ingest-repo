...
common_task = CommonTask(dag_id='notebook_v1', dag_params={})
lineage_task = LineageTask(dag_id='notebook_v1', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 436,
            'flow_name': 'Notebook_v1',
            'flow_key': 'notebook_v1',
            'bh_project_id': 300,
    ...
    def create_databricks_cluster_create_compute_2464c6530(**context):
    ...
    create_compute_2464c6530 = PythonOperator(
        task_id='create_compute_2464c6530',
        python_callable=create_databricks_cluster_create_compute_2464c6530,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_2464c6530",
        "job_config": {
            "job_type": "notebook",
            "notebook_path": "/Workspace/Users/sasikumar@bighammer.ai/verato_1",
            "parameters": {}
        },
        "compute_xcom_key": "return_value"
    }
    submit_notebook_b9d4a28fc = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_b9d4a28fc',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_2464c6530", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_2464c6530 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_e834b4c9e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_e834b4c9e',
    ...
    start_flow_task >> create_compute_2464c6530
    create_compute_2464c6530 >> submit_notebook_b9d4a28fc
    submit_notebook_b9d4a28fc >> delete_compute_e834b4c9e
    create_compute_2464c6530 >> delete_compute_e834b4c9e
    delete_compute_e834b4c9e >> end_flow_task

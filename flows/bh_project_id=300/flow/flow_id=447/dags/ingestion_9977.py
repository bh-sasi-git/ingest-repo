...
common_task = CommonTask(dag_id='ingestion_9977', dag_params={})
lineage_task = LineageTask(dag_id='ingestion_9977', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 447,
            'flow_name': 'ingestion_9977',
            'flow_key': 'ingestion_9977',
            'bh_project_id': 300,
    ...
    def create_databricks_cluster_create_compute_944437e5a(**context):
    ...
    create_compute_944437e5a = PythonOperator(
        task_id='create_compute_944437e5a',
        python_callable=create_databricks_cluster_create_compute_944437e5a,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_944437e5a",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_be6478f71 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_be6478f71',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_944437e5a", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_944437e5a or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_b98430099 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_b98430099',
    ...
    start_flow_task >> create_compute_944437e5a
    create_compute_944437e5a >> submit_notebook_be6478f71
    submit_notebook_be6478f71 >> delete_compute_b98430099
    create_compute_944437e5a >> delete_compute_b98430099
    delete_compute_b98430099 >> end_flow_task

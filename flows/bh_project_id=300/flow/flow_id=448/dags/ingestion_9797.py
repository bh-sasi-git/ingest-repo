...
common_task = CommonTask(dag_id='ingestion_9797', dag_params={})
lineage_task = LineageTask(dag_id='ingestion_9797', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 448,
            'flow_name': 'INGESTION_9797',
            'flow_key': 'ingestion_9797',
            'bh_project_id': 300,
    ...
    def create_databricks_cluster_create_compute_7d8415f1f(**context):
    ...
    create_compute_7d8415f1f = PythonOperator(
        task_id='create_compute_7d8415f1f',
        python_callable=create_databricks_cluster_create_compute_7d8415f1f,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_7d8415f1f",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_0c04c3937 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_0c04c3937',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_7d8415f1f", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_7d8415f1f or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_7284cfbb9 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_7284cfbb9',
    ...
    start_flow_task >> create_compute_7d8415f1f
    create_compute_7d8415f1f >> submit_notebook_0c04c3937
    submit_notebook_0c04c3937 >> delete_compute_7284cfbb9
    create_compute_7d8415f1f >> delete_compute_7284cfbb9
    delete_compute_7284cfbb9 >> end_flow_task

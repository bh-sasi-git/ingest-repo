...
common_task = CommonTask(dag_id='ingestion_77', dag_params={})
lineage_task = LineageTask(dag_id='ingestion_77', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 446,
            'flow_name': 'INgestion_77',
            'flow_key': 'ingestion_77',
            'bh_project_id': 300,
    ...
    def create_databricks_cluster_create_compute_8c645f1fc(**context):
    ...
    create_compute_8c645f1fc = PythonOperator(
        task_id='create_compute_8c645f1fc',
        python_callable=create_databricks_cluster_create_compute_8c645f1fc,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_8c645f1fc",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_fc846a52c = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_fc846a52c',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_8c645f1fc", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_8c645f1fc or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_27d4159e5 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_27d4159e5',
    ...
    start_flow_task >> create_compute_8c645f1fc
    create_compute_8c645f1fc >> submit_notebook_fc846a52c
    submit_notebook_fc846a52c >> delete_compute_27d4159e5
    create_compute_8c645f1fc >> delete_compute_27d4159e5
    delete_compute_27d4159e5 >> end_flow_task
